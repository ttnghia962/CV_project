...
def Main():
    ...
    for Loop in range(0, 1000000):
        ...
        account_balance = random.randint(1,1000000000)
        print(f'({Loop + 1}) Username: {Username} || Password: {Password} || account_balance: {account_balance}$')
        sheet.cell(row=Loop+2, column=1, value=Username)
        sheet.cell(row=Loop+2, column=2, value=Password)
        sheet.cell(row=Loop+2 , column=3, value=str(account_balance) + '$')
